...
/**
 * The function checks if a resource exists on web drive and returns the item
 * 
 ...
 */
Json KoofrApiImpl::checkIfRemoteResourceExists(std::string resourceTitle,std::string mimeType,std::string parentId)
{
    long ret;
    Json resourceObject;
    ...
    std::string url = m_api_root_url+"drive/v2/files?q=" + http.Escape(query) + "&fields=" + http.Escape(queryFields)+"&orderBy=createdDate";
 ...
 */
Json::Array KoofrApiImpl::getResourcesUnderFolder(std::string parentId)
{
    long ret;
...
/**
 * Downloads a file from web drive to a local folder
 * 
 * @param localPath the local path to save the file to
 * @param fileId the id of the file in google drive to download
 */
bool KoofrApiImpl::downloadFile(std::string localPath, std::string fileId)
{
    long ret;
...
void KoofrApiImpl::downloadChanges(int *dialog_action)
{
 ...
 */
Json KoofrApiImpl::uploadFile(std::string path, std::string filename, std::string mimeType, std::string parentId, std::string fileId)
{
    long ret;
    StdioFile file(path);
    Json obj;
    ...
    if ( ret >= 400 && ret < 500 )
    {
        debugPrintf("uploadFile: Could not post meta data..%l\n", ret);
        // This could mean the internet disconnected, or the server crashed or whatever
        // Possibly exit the application?
        return obj;
    }
